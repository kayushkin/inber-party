...
- [~] **URGENT: Fix Persistent WebSocket Connection Churn During E2E Tests (Round 4)** — WebSocket connection churn is STILL occurring during E2E tests despite multiple optimization attempts. Observed during current test run with constant "WebSocket client connected/disconnected" cycles happening every few seconds. Previous optimizations have not resolved the core issue. Need to implement more aggressive connection pooling or fundamentally restructure WebSocket connection management during test environments. This is causing significant performance degradation and test instability.
