...
// isCleanAgentID reports whether id is a single, contained path element and so
// is safe to join onto AgentsDir. An agent id names one directory directly
// under AgentsDir; it is never a path. The check mirrors logstack's own
// store.isCleanName: reject the empty, self and parent names outright, reject
// anything carrying a separator or a NUL, and finally require that reducing the
// value to its last element does not change it — which collapses any residual
// traversal a raw separator check would miss.
//
// This exists because agentID arrives from the `agent` query parameter of the
// /api/conversations endpoints and is joined straight onto AgentsDir. The HTTP
// router cleans the escaped path, not the decoded value the handler receives,
// so "..%2f..%2fx" reaches here as "../../x" and reads outside AgentsDir.
func isCleanAgentID(id string) bool {
	if id == "" || id == "." || id == ".." {
		return false
	}
	if strings.ContainsRune(id, '/') || strings.ContainsRune(id, '\\') || strings.ContainsRune(id, 0) {
		return false
	}
...
func (lc *LogstackClient) GetAgentConversations(agentID string, limit int) ([]Conversation, error) {
	...
	if !isCleanAgentID(agentID) {
		return nil, fmt.Errorf("invalid agent id %q: not a single path element", agentID)
	}
